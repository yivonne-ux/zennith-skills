...
- v1.5 (2026-03-30): Added from v13 batch audit: DIM 3 non-pink sparkle check (gold/brown glitter = off-brand for Mirra), DIM 9 stricter viral trace (generic internet phrases like "emotionally attached to food" or "folding a fitted sheet" = NOT traced, need actual source handle), DIM 2 brand product recognizability (food in quiz must be CLEARLY the brand's packaging, not generic), DIM 7 logo-text overlap pre-check (if bottom 20% has text → logo must be TR).
