...
|----------|--------|------|-------|---------------|------|
| **YouTube** | Data API v3 | **Free** (10K units/day) | Scout | Trend research, competitor channels, video metadata | Low |
| **Pinterest** | API v5 (OAuth2) | **Free** (needs dev app) | Scout | Style/mood boards, visual inspiration, design trends | Low |
| **TikTok** | Apify scraper or `TikTokApi` lib | **~RM 200/mo** via Apify | Dreami | Viral formats, hooks, hashtag trends | Medium |
| **Instagram** | Own account via `instagrapi` + Apify for competitors | **~RM 100/mo** via Apify | Dreami | Own engagement + competitor hashtags | Medium |
| **Facebook** | Meta Graph API (public pages only) | **Free** | Dreami | Own brand page monitoring | Low |
| **X.com** | **Skip** — free tier is write-only | N/A | — | Not practical without $200/mo paid tier | — |
| **Google Trends** | Free API | **Free** | Scout | Keyword trends (Malaysian food, vegan, etc.) | Low |
...
Cron picks up jobs every 30 min
    ↓
Routes to: Scout (research platforms) or Dreami (social platforms)
    ↓
...
- Scout stores competitive intel in RAG memory
- Dreami stores engagement patterns
- Dreami uses insights for next campaign brief
- Dreami adapts visual style based on trend data
