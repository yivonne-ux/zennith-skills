...
This is the single entry point for all creative inputs into the GAIA content factory. Whether Jenn sends a Pinterest link via WhatsApp, Scout discovers a competitor ad, or a studio upload comes in via API — it all flows through here.
...
- visual-reference (Pinterest, Behance, Dribbble) -> Dreami reverse-prompts via Gemini Vision
- competitor ad -> Scout for competitive analysis
- tutorial/article -> Scout + Dreami for technique extraction
- product -> Dreami for product photography workflow

### Image (type: image)
- Dreami reverse-prompts via Gemini Vision API (gemini-2.5-flash)
