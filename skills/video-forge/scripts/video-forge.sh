...
# --- Python detection (faster-whisper needs python3.13 on macOS) ---
...
cmd_caption() {
  ...
  $PY - "$abs_input" "$srt_file" "$whisper_lang_arg" "$word_level" <<'PYEOF'
import sys, os, json
...
PYEOF
  ...
  # ffmpeg ASS filter parser chokes on spaces/colons — ALWAYS use temp file
  local tmp_ass="/tmp/vf_caption_$$.ass"
  cp "$ass_file" "$tmp_ass" 2>/dev/null || {
    error "Failed to copy ASS file to temp: $ass_file"
    echo ""
    return 1
  }
  # Symlink input to temp if path has spaces (ASS filter breaks, -i might too)
  local caption_input="$abs_input"
  local tmp_input=""
  case "$abs_input" in
    *" "*)
      tmp_input="/tmp/vf_caption_input_$$.mp4"
      ln -sf "$abs_input" "$tmp_input"
      caption_input="$tmp_input"
      ;;
  esac
  local tmp_output="/tmp/vf_caption_out_$$.mp4"
  ffmpeg -i "$caption_input" \
    -vf "ass=${tmp_ass}" \
    -c:v h264_videotoolbox -b:v 8M \
    -c:a aac -b:a 192k \
    -y "$tmp_output" \
    -loglevel warning 2>> "$LOG_FILE"
  rm -f "$tmp_ass"
  [ -n "$tmp_input" ] && rm -f "$tmp_input"
  if [ -f "$tmp_output" ]; then
    mv "$tmp_output" "$captioned_file"
  fi

  # Clean up temp segments JSON
  rm -f "${outdir}/${base}_segments.json"

  if [ ! -f "$captioned_file" ]; then
    error "Caption burn failed — output file not created: $captioned_file"
    echo ""
    return 1
  fi
...
cmd_brand() {
  ...
  if [ ! -f "$branded_file" ]; then
    error "Brand overlay failed — output file not created: $branded_file"
    echo ""
    return 1
  fi
...
cmd_music() {
  ...
  if [ ! -f "$mixed_file" ]; then
    error "Music mix failed — output file not created: $mixed_file"
    echo ""
    return 1
  fi
...
")
  ...
  if [ -z "$vfilters" ] && [ -z "$atempo_filter" ]; then
    ...
    if [ -n "$vfilters" ]; then
      ...
      local speed_pts
      speed_pts=$(python3 -c "print(1.0/float('$speed'))")
      ffmpeg -i "$abs_input" \
  ...
  if [ ! -f "$effects_file" ]; then
    error "Effects failed — output file not created: $effects_file"
    echo ""
    return 1
  fi
...
cmd_assemble() {
  ...
  if [ "$transition" = "none" ]; then
    ...
    # NOTE: All clips are normalized to same resolution (first clip's size) via scale+pad
    local filter_graph
...
for c in clips:
    ...
    try:
        ...
        sizes.append((1080, 1920))

# Use first clip's resolution as target
tw, th = sizes[0]

# Check if normalization needed
need_norm = any(s != (tw, th) for s in sizes)

# Build normalize filters: scale to fit + pad to exact size + set fps/format
norm_parts = []
for i in range(n):
    tag_v = f'[n{i}v]'
    tag_a = f'[n{i}a]'
    if need_norm:
        norm_parts.append(f'[{i}:v]scale={tw}:{th}:force_original_aspect_ratio=decrease,pad={tw}:{th}:(ow-iw)/2:(oh-ih)/2:black,fps=30,format=yuv420p{tag_v}')
    else:
        norm_parts.append(f'[{i}:v]fps=30,format=yuv420p{tag_v}')
    norm_parts.append(f'[{i}:a]aformat=sample_rates=44100:channel_layouts=stereo{tag_a}')

if n == 2:
    offset = max(0, durations[0] - trans_dur)
    vf = f'[n0v][n1v]xfade=transition={xfade_type}:duration={trans_dur}:offset={offset}[vout]'
    af = f'[n0a][n1a]acrossfade=d={trans_dur}[aout]'
    print(';'.join(norm_parts) + ';' + vf + ';' + af)
else:
    vf_parts = []
    af_parts = []
    cumulative = 0
    ...
    print(';'.join(norm_parts + vf_parts + af_parts))
  ...
  case "$output_type" in
    ...
    broll)
      ...
      if [ -n "$grade_preset" ]; then
        current=$(cmd_effects "$current" --grade "$grade_preset" --output "$outdir")
      else
        current=$(cmd_effects "$current" --output "$outdir")
      fi
    ...
    aroll)
      ...
      info "--- Pipeline: aroll ---"
      local captioned
      captioned=$(cmd_caption "$current" --style tiktok --word-level --output "$outdir") || true
      if [ -n "$captioned" ] && [ -f "$captioned" ]; then current="$captioned"; fi
    ...
    promotion)
      ...
      local captioned
      captioned=$(cmd_caption "$current" --style bold --output "$outdir") || true
      if [ -n "$captioned" ] && [ -f "$captioned" ]; then current="$captioned"; fi
    ...
    ugc)
      # Chain: effects(grain-light) -> caption(tiktok-native) -> export(9:16 only)
      # NOTE: removed --shaky (AI-generated clips already have natural movement; double-shake kills quality)
      info "--- Pipeline: ugc ---"
      current=$(cmd_effects "$current" --grain light --output "$outdir")
      local captioned
      captioned=$(cmd_caption "$current" --style tiktok --word-level --output "$outdir") || true
      if [ -n "$captioned" ] && [ -f "$captioned" ]; then
        current="$captioned"
      else
        info "Caption step skipped (no libass or whisper), continuing with effects-only"
      fi
    ...
    lofi)
      ...
      if has_audio "$abs_input"; then
        ...
        if [ -n "$speech_check" ] && [ "$speech_check" != "0" ] && [ "$speech_check" != "N/A" ]; then
          local captioned
          captioned=$(cmd_caption "$current" --style clean --output "$outdir") || true
          if [ -n "$captioned" ] && [ -f "$captioned" ]; then current="$captioned"; fi
        fi
      fi
