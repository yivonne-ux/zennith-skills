...
load_env() {
  ...
  if [ -z "${LUMA_API_KEY:-}" ]; then
    LUMA_API_KEY=$(grep 'LUMA_API_KEY=' "$HOME/.zshrc" 2>/dev/null | head -1 | sed "s/.*LUMA_API_KEY=['\"]*//" | sed "s/['\"].*//" || true)
    export LUMA_API_KEY
    if [ -z "${LUMA_API_KEY:-}" ] && [ -f "$HOME/.openclaw/secrets/luma.env" ]; then
      LUMA_API_KEY=$(grep '^LUMA_API_KEY=' "$HOME/.openclaw/secrets/luma.env" | head -1 | cut -d= -f2- | sed "s/^['\"]//;s/['\"]$//")
      export LUMA_API_KEY
    fi
  fi
}
...
# ============================================================
# LUMA PROVIDER (fal.ai + direct Luma API)
# ============================================================
# Models: ray-2 (quality, ~$0.50/5s), ray-2-flash (fast, ~$0.20/5s)
# fal.ai endpoints: fal-ai/luma-dream-machine/ray-2, ray-2-flash
# Direct API: https://api.lumalabs.ai/dream-machine/v1/generations

LUMA_FAL_T2V_RAY2="fal-ai/luma-dream-machine/ray-2"
LUMA_FAL_T2V_FLASH="fal-ai/luma-dream-machine/ray-2-flash"
LUMA_FAL_I2V_RAY2="fal-ai/luma-dream-machine/ray-2/image-to-video"
LUMA_FAL_I2V_FLASH="fal-ai/luma-dream-machine/ray-2-flash/image-to-video"
LUMA_DIRECT_URL="https://api.lumalabs.ai/dream-machine/v1/generations"

luma_check_key() {
  # Prefer FAL_API_KEY (consistent with Wan/Kling fal.ai usage)
  # Fall back to LUMA_API_KEY for direct Luma API
  if [ -z "${FAL_API_KEY:-}" ] && [ -z "${LUMA_API_KEY:-}" ]; then
    echo "ERROR: FAL_API_KEY or LUMA_API_KEY not set"
    echo "Set in ~/.openclaw/.env or environment"
    echo "FAL_API_KEY uses fal.ai proxy (recommended)"
    echo "LUMA_API_KEY uses direct Luma API"
    exit 1
  fi
}

# Resolve fal.ai model ID from user --model option
luma_resolve_model() {
  local mode="$1"    # t2v or i2v
  local model="$2"   # user input: ray-2, ray-2-flash, flash, etc.

  case "$model" in
    ray-2-flash|flash|fast)
      if [ "$mode" = "i2v" ]; then
        echo "$LUMA_FAL_I2V_FLASH"
      else
        echo "$LUMA_FAL_T2V_FLASH"
      fi
      ;;
    ray-2|""|*)
      if [ "$mode" = "i2v" ]; then
        echo "$LUMA_FAL_I2V_RAY2"
      else
        echo "$LUMA_FAL_T2V_RAY2"
      fi
      ;;
  esac
}

# Cost estimate based on model, duration, resolution
luma_cost_estimate() {
  local model="$1"
  local duration="$2"
  local resolution="$3"

  local base_cost="0.50"
  case "$model" in
    *flash*) base_cost="0.20" ;;
  esac

  # Duration multiplier: 9s = 2x
  local dur_mult=1
  if [ "$duration" = "9s" ] || [ "$duration" = "9" ]; then
    dur_mult=2
  fi

  # Resolution multiplier: 720p = 2x, 1080p = 4x
  local res_mult=1
  case "$resolution" in
    720p)  res_mult=2 ;;
    1080p) res_mult=4 ;;
  ...
  python3 -c "print('%.2f' % ($base_cost * $dur_mult * $res_mult))"
}

# Normalize duration to Luma-valid values (5s or 9s)
luma_normalize_duration() {
  local seconds="$1"
  case "$seconds" in
    1|2|3|4|5) echo "5s" ;;
    6|7|8|9)   echo "9s" ;;
...
# Normalize aspect ratio to Luma-valid values
luma_normalize_ratio() {
  local ratio="$1"
  case "$ratio" in
    16:9|9:16|4:3|3:4|21:9|9:21) echo "$ratio" ;;
    1:1) echo "4:3" ;;  # Luma doesn't support 1:1, closest is 4:3
    *)   echo "16:9" ;;
  esac
}
...
luma_text2video() {
  ...
  if [ -z "$OPT_PROMPT" ]; then
    echo "ERROR: prompt is required"
    echo "Usage: video-gen.sh luma text2video \"prompt\" [--model ray-2] [--duration 5] [--aspect-ratio 9:16] [--brand pinxin-vegan]"
    exit 1
  fi

  local model_name="${OPT_MODEL:-ray-2}"
  local duration
  duration=$(luma_normalize_duration "$OPT_DURATION")
  local ratio
  ratio=$(luma_normalize_ratio "$OPT_ASPECT_RATIO")
  local resolution="${OPT_RESOLUTION:-540p}"
  local output="$OPT_OUTPUT"
  local brand="$OPT_BRAND"
  ...
  local fal_model
  fal_model=$(luma_resolve_model "t2v" "$model_name")

  local cost
  cost=$(luma_cost_estimate "$fal_model" "$duration" "$resolution")

  echo "=== Luma Text-to-Video ==="
  echo "Prompt:     $prompt"
  echo "Duration:   $duration"
  echo "Ratio:      $ratio"
  echo "Resolution: $resolution"
  echo "Model:      $fal_model"
  echo "Est cost:   ~\$$cost"
  echo ""

  log "luma text2video: model=$fal_model duration=$duration ratio=$ratio resolution=$resolution brand=$brand"
  log "Prompt: $prompt"

  if [ "$OPT_DRY_RUN" -eq 1 ]; then
    echo "[DRY RUN] Would POST to: ${FAL_BASE_URL}/${fal_model}"
    exit 0
  fi

  # Prefer fal.ai, fall back to direct Luma API
  if [ -n "${FAL_API_KEY:-}" ]; then
    luma_generate_fal "$fal_model" "$prompt" "" "$duration" "$ratio" "$resolution" "$output" "$brand"
  elif [ -n "${LUMA_API_KEY:-}" ]; then
    luma_generate_direct "$model_name" "$prompt" "" "$duration" "$ratio" "$resolution" "$output" "$brand"
  else
    luma_check_key  # will exit with error
  fi
}
...
luma_image2video() {
  ...
  if [ -z "$OPT_IMAGE" ]; then
    echo "ERROR: --image URL or file path is required for image2video"
    echo "Usage: video-gen.sh luma image2video --image \"url_or_path\" [--prompt \"...\"] [--model ray-2] [--duration 5]"
    exit 1
  fi
  ...
  # If local file, upload to fal.ai CDN
  local resolved_image="$image_url"
  if [ -f "$image_url" ]; then
    echo "Local file detected, uploading to fal.ai CDN..."
    local cdn_url
    cdn_url=$(upload_to_fal_cdn "$image_url" 2>/dev/null)
    if [ -n "$cdn_url" ] && [ "${cdn_url#http}" != "$cdn_url" ]; then
      echo "Uploaded: $cdn_url"
      resolved_image="$cdn_url"
    else
      echo "fal CDN upload failed, encoding as base64..."
      resolved_image=$(encode_image_for_fal "$image_url")
    fi
  fi

  local fal_model
  fal_model=$(luma_resolve_model "i2v" "$model_name")

  local cost
  cost=$(luma_cost_estimate "$fal_model" "$duration" "$resolution")

  echo "=== Luma Image-to-Video ==="
  echo "Image:      ${OPT_IMAGE}"
  echo "Prompt:     ${prompt:-<none>}"
  echo "Duration:   $duration"
  echo "Ratio:      $ratio"
  ...
  log "luma image2video: image=$image_url model=$fal_model duration=$duration ratio=$ratio resolution=$resolution brand=$brand"
  ...
  if [ -n "${FAL_API_KEY:-}" ]; then
    luma_generate_fal "$fal_model" "$prompt" "$resolved_image" "$duration" "$ratio" "$resolution" "$output" "$brand"
  elif [ -n "${LUMA_API_KEY:-}" ]; then
    luma_generate_direct "$model_name" "$prompt" "$resolved_image" "$duration" "$ratio" "$resolution" "$output" "$brand"
  else
    luma_check_key
  fi
}

# Generate via fal.ai proxy (reuses wan_submit/wan_poll/wan_extract_video_url)
luma_generate_fal() {
  local fal_model="$1"
  local prompt="$2"
  local image_url="$3"
  ...
  wan_check_key  # checks FAL_API_KEY
...
d = {'prompt': sys.argv[1]}
if sys.argv[2]:
    d['image_url'] = sys.argv[2]
d['duration'] = sys.argv[3]
d['aspect_ratio'] = sys.argv[4]
d['resolution'] = sys.argv[5]
print(json.dumps(d))
" "$prompt" "$image_url" "$duration" "$ratio" "$resolution")

  echo "Submitting to fal.ai queue..."
  local request_id
  request_id=$(wan_submit "$fal_model" "$payload")

  echo "Job submitted: $request_id"
  log "Submitted: luma request=$request_id model=$fal_model"

  wan_poll "$fal_model" "$request_id" 600

  local video_url
  video_url=$(wan_extract_video_url "$fal_model" "$request_id")

  if [ -z "$video_url" ]; then
    echo "ERROR: Could not extract video URL from result"
    log "ERROR: luma no video URL for request=$request_id"
    exit 1
  fi
  ...
  post_generate "luma" "$request_id" "$output" "$brand" "${OPT_PROMPT:-$prompt}"
  echo "$output"
}

# Generate via direct Luma API (fallback when no FAL_API_KEY)
luma_generate_direct() {
  local model_name="$1"
  local prompt="$2"
  local image_url="$3"
  local duration="$4"
  ...
  if [ -z "${LUMA_API_KEY:-}" ]; then
    echo "ERROR: LUMA_API_KEY not set for direct Luma API"
    exit 1
  fi
...
if sys.argv[6]:
    d['keyframes'] = {'frame0': {'type': 'image', 'url': sys.argv[6]}}
print(json.dumps(d))
" "$prompt" "$model_name" "$duration" "$ratio" "$resolution" "$image_url")

  echo "Submitting to Luma API directly..."
  local response
  response=$(curl -s -X POST "$LUMA_DIRECT_URL" \
    -H "Authorization: Bearer ${LUMA_API_KEY}" \
    -H "Content-Type: application/json" \
    -d "$payload")

  local gen_id
  gen_id=$(echo "$response" | python3 -c "import sys,json; print(json.load(sys.stdin).get('id',''))" 2>/dev/null)

  if [ -z "$gen_id" ]; then
    echo "ERROR: Failed to create Luma generation"
    echo "$response"
    log "ERROR: luma direct create failed — $response"
    exit 1
  fi

  echo "Generation created: $gen_id"
  log "Luma direct generation: $gen_id"

  # Poll for completion
  echo "Polling for completion (every 10s, max 10min)..."
  local max_attempts=60
  local attempt=0
  ...
  while [ $attempt -lt $max_attempts ]; do
    ...
    local status_resp
    status_resp=$(curl -s -X GET "${LUMA_DIRECT_URL}/${gen_id}" \
      -H "Authorization: Bearer ${LUMA_API_KEY}")

    local state
    state=$(echo "$status_resp" | python3 -c "import sys,json; print(json.load(sys.stdin).get('state','unknown'))" 2>/dev/null)
...
d = json.load(sys.stdin)
assets = d.get('assets', {})
print(assets.get('video', ''))
" 2>/dev/null)

      if [ -z "$video_url" ]; then
        echo "ERROR: No video URL in completed generation"
        echo "$status_resp"
        exit 1
      fi
      ...
      if [ -z "$output" ]; then
        local ts
        ts=$(date '+%Y%m%d_%H%M%S')
        output="${OUTPUT_DIR}/luma-direct-${ts}.mp4"
      fi
      ...
      echo ""
      echo "=== Complete (direct Luma API) ==="
      echo "Video saved: $output"
      echo "File size:   ${file_size} bytes"
      log "Video saved: $output (${file_size} bytes, gen=$gen_id)"

      post_generate "luma" "$gen_id" "$output" "$brand" "${OPT_PROMPT:-$prompt}"
      echo "$output"
      return 0

    elif [ "$state" = "failed" ]; then
      local fail_reason
      fail_reason=$(echo "$status_resp" | python3 -c "import sys,json; print(json.load(sys.stdin).get('failure_reason','unknown'))" 2>/dev/null)
      echo "FAILED: $fail_reason"
      log "FAILED: luma direct $gen_id — $fail_reason"
      exit 1
    fi
  done

  echo "TIMEOUT: Luma generation timed out after 10 minutes"
  echo "Check status: video-gen.sh luma status $gen_id"
  log "TIMEOUT: luma direct $gen_id"
  exit 1
}
...
luma_status() {
  local request_id="${1:-}"
  if [ -z "$request_id" ]; then
    echo "ERROR: request_id or generation_id required"
    echo "Usage: video-gen.sh luma status <id>"
    exit 1
  fi

  # Try fal.ai first (check both ray-2 and flash models)
  if [ -n "${FAL_API_KEY:-}" ]; then
    local status_resp
    status_resp=$(curl -s -X GET "${FAL_BASE_URL}/${LUMA_FAL_T2V_RAY2}/requests/${request_id}/status" \
      -H "Authorization: Key ${FAL_API_KEY}" 2>/dev/null)

    local status
    status=$(echo "$status_resp" | python3 -c "import sys,json; print(json.load(sys.stdin).get('status',''))" 2>/dev/null)

    if [ -n "$status" ]; then
      echo "Provider: fal.ai (Luma Ray 2)"
  ...
  if [ -n "${LUMA_API_KEY:-}" ]; then
    local resp
    resp=$(curl -s -X GET "${LUMA_DIRECT_URL}/${request_id}" \
      -H "Authorization: Bearer ${LUMA_API_KEY}" 2>/dev/null)

    local state
    state=$(echo "$resp" | python3 -c "import sys,json; print(json.load(sys.stdin).get('state',''))" 2>/dev/null)

    if [ -n "$state" ]; then
      echo "Provider: Luma Direct API"
      echo "ID:       $request_id"
      echo "State:    $state"
      local video_url
      video_url=$(echo "$resp" | python3 -c "import sys,json; print(json.load(sys.stdin).get('assets',{}).get('video',''))" 2>/dev/null)
      if [ -n "$video_url" ]; then
  ...
  echo "ERROR: Could not find generation $request_id"
  echo "Ensure FAL_API_KEY or LUMA_API_KEY is set"
  exit 1
}
...
luma_download() {
  ...
  if [ -z "$request_id" ]; then
    echo "ERROR: request_id or generation_id required"
    echo "Usage: video-gen.sh luma download <id> [--output path.mp4]"
    exit 1
  fi
  ...
  # Try fal.ai first
  if [ -n "${FAL_API_KEY:-}" ]; then
    video_url=$(wan_extract_video_url "$LUMA_FAL_T2V_RAY2" "$request_id" 2>/dev/null)
    if [ -z "$video_url" ]; then
      video_url=$(wan_extract_video_url "$LUMA_FAL_I2V_RAY2" "$request_id" 2>/dev/null)
    fi
    if [ -z "$video_url" ]; then
      video_url=$(wan_extract_video_url "$LUMA_FAL_T2V_FLASH" "$request_id" 2>/dev/null)
    fi
    if [ -z "$video_url" ]; then
      video_url=$(wan_extract_video_url "$LUMA_FAL_I2V_FLASH" "$request_id" 2>/dev/null)
    fi
  fi

  # Try direct Luma API
  if [ -z "$video_url" ] && [ -n "${LUMA_API_KEY:-}" ]; then
    video_url=$(curl -s -X GET "${LUMA_DIRECT_URL}/${request_id}" \
      -H "Authorization: Bearer ${LUMA_API_KEY}" | \
      python3 -c "import sys,json; print(json.load(sys.stdin).get('assets',{}).get('video',''))" 2>/dev/null)
  fi

  if [ -z "$video_url" ]; then
    echo "ERROR: No completed video found for $request_id"
    exit 1
  fi

  local output="${OPT_OUTPUT:-${OUTPUT_DIR}/luma-${request_id}.mp4}"
  curl -s -o "$output" "$video_url"
  echo "Downloaded: $output"
  log "Downloaded: luma $request_id -> $output"
}
...
PROVIDERS:
  ...
  luma     Luma Ray 2 via fal.ai (text2video, image2video, status, download)
...
USAGE — Provider:
  ...
  video-gen.sh luma text2video "prompt" [--model ray-2] [--duration 5] [--aspect-ratio 9:16] [--resolution 540p]
  video-gen.sh luma image2video --image "url_or_path" [--prompt "..."] [--model ray-2-flash] [--duration 5]
  video-gen.sh luma status <request_id>
  video-gen.sh luma download <request_id> [--output path.mp4]
...
COST ESTIMATES:
  ...
  Sora generate:         ~$0.50
  Luma Ray 2 (5s/540p):  ~$0.50
  Luma Ray 2 Flash:      ~$0.20
  Luma Ray 2 (9s/720p):  ~$2.00

ENVIRONMENT VARIABLES:
  KLING_ACCESS_KEY, KLING_SECRET_KEY  Kling AI credentials
  FAL_API_KEY                         fal.ai API key (for Wan, Luma via fal.ai)
  LUMA_API_KEY                        Luma API key (direct API, fallback if no FAL_API_KEY)
...
main() {
  ...
  case "${1:-}" in
    ...
    luma)
      shift
      case "${1:-}" in
        text2video)  shift; luma_text2video "$@" ;;
        image2video) shift; luma_image2video "$@" ;;
        status)      shift; luma_status "$@" ;;
