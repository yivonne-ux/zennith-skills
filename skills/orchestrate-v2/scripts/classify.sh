...
classify_override() {
  ...
  if echo "$task" | grep -qiE '^(@|/)(taoz|dreami|scout)'; then
    local direct_agent
    direct_agent=$(echo "$task" | grep -oiE '(taoz|dreami|scout)' | head -1 | tr '[:upper:]' '[:lower:]')
    echo "$direct_agent"
    return 0
  fi
  ...
  # PM FRAMEWORKS / STRATEGIC ANALYSIS → ATHENA (must be before Dreami's "brand voice" pattern)
  if echo "$task" | grep -qiE '(brand.?voice.?(consist|audit|analy|check|score)|a.?b.?test.?(analy|result|signif|evaluat)|north.?star.?(metric)?|customer.?journey.?(map|analy)|pre.?mortem|go.?to.?market.?(strateg|plan)|gtm.?(strateg|plan|launch)|beachhead.?(segment)?|ideal.?customer.?profile|growth.?loop.?(analy|design)|cohort.?(analy|retent)|incrementality.?(test)?|market.?siz)'; then
    echo "main"
    return 0
  fi

  # PRICING OPTIMIZATION → HERMES (must be before Taoz's "optimize" system pattern)
  if echo "$task" | grep -qiE '(optimiz.*(pric|margin|cost|revenue|subscription)|pricing.*(optim|best|ideal)|best.?pric|ideal.?pric)'; then
    echo "dreami"
    return 0
  fi

  # SHOPIFY / E-COMMERCE OPERATIONS → HERMES (merchant)
  # Excludes "build/create shopify webhook/integration/app" (CODE tier tasks)
  if echo "$task" | grep -qiE '(upload.*(product|image|inventory).*(to |into )?(store|shop|shopify)|product.*(listing|catalog).*(shopify|store)|shopify.*(upload|product|order|inventory|customer|listing)|e.?commerce.*(upload|manage|inventory)|bulk.?(upload|import).*(product|sku))' && ! echo "$task" | grep -qiE '(build|create|code|develop|fix|debug).*(shopify|webhook|integration|app|plugin)'; then
    echo "dreami"
    return 0
  fi

  # META ADS MANAGEMENT → HERMES (campaign setup, budgets, links, specific ad checks)
  # Hermes manages Meta ads platforms (Meta Ads Manager, Ads API, budget setup)
  # EXCLUDES: performance analysis → Athena, ad generation → Iris, review/audit → Argus
  if echo "$task" | grep -qiE '(meta.?ads?.?(performance|spend|report|result|check|status|health))' && echo "$task" | grep -qiE '(set|create|link|setup|budget|spend|manager|api|campaign|ad set)' && ! echo "$task" | grep -qiE '(how (are|is) .*(doing|working)|how (is|are) .*(perform|performing)|are .*(working|doing|going)|check.*(ad|ads|campaign).*(doing|working|going)|pull.*(number|stat|data)|show.*(number|stat|data)|how many|across all|all.*performance|which.*best|performance (review|this month|last|data|stats)|multi.?variable|cohort|retent|journ)'; then
    echo "dreami"
    return 0
  fi

  # LIP SYNC / VOICE-OVER / DUBBING → IRIS (video pipeline lead)
  if echo "$task" | grep -qiE '(lip.?sync|voice.?over|voiceover|dub.*(video|clip)|talking.?head|tts.*(video|sync)|sync.*(lip|audio|voice))'; then
    echo "dreami"
    return 0
  fi
  ...
  # AD COPY / COPYWRITING → DREAMI (must be before brand studio to prevent "ad copy" matching image ad gen)
  # Catches: "generate 3 ad copy variants for mirra", "write PAS formula ad copy", "ad copy for bento"
  # NOTE: "comparison ads" WITHOUT "copy" → dreami (image gen). "comparison ad copy" → dreami (text)
  if echo "$task" | grep -qiE '(ad.?cop(y|ies)|copywriting|marketing.?copy|brand.?copy|sales.?copy|hook.*(write|creat|generat)|caption.*(write|creat|generat)|comparison.?ad.*cop(y|ies))' && ! echo "$task" | grep -qiE '(video|clip|reel|animation|compile)'; then
    echo "dreami"
    return 0
  fi

  # BRAND STUDIO: ad/image generation with brand awareness → dreami
  ...
  # EXCLUDES: ugc video/reel/clip → dreami (video scripting)
  if echo "$task" | grep -qiE '(create|generate|make).*(mirra|pinxin|wholey|serein|rasaya|gaia).*(ad|ads|image|visual|poster|banner|post|catalog|carousel|grid|comparison|hero|lifestyle|testimonial|persona|sales|promo|deal|urgency|price|raw|meme|product|ugc)|(create|generate|make).*[0-9]*.*(ad|ads|image|visual|poster|banner).*(mirra|pinxin|wholey|serein|rasaya|gaia)|brand.?studio|comparison.?ad|hero.?ad|grid.?ad|lifestyle.?ad|bento.?ugc' && ! echo "$task" | grep -qiE 'ugc.*(video|reel|clip|tiktok|sora|kling|wan)|video.*(ugc|reel)|sora.*(ugc|reel)'; then
    echo "dreami"
    return 0
  fi

  # CONVERSATIONAL / DISCUSSION / PLANNING → RELAY (Zenni handles herself with GPT-5.4)
  # When Jenn is TALKING to Zenni (discussion, planning, thinking, asking opinions)
  # NOT building/fixing/deploying — just having a conversation
  # Must be BEFORE the tool/tutorial pattern to prevent "look into his persona" matching CODE tier
  #
  # PRE-CHECK: If the message contains action keywords, it is NOT pure conversation.
  # Skip RELAY and let it fall through to the correct agent pattern below.
  if echo "$task" | grep -qiE '(what do you (think|suggest|recommend)|how should we|what should we (do|focus|prioritize|try)|your (opinion|thought|view|take|recommendation)|do you (think|know|remember|understand|agree)|can you (explain|tell me|help me understand|discuss)|let.?s (talk|discuss|plan|think|figure)|i (think|believe|want|need) (you to |u to )?(know|understand|think|plan|discuss|consider)|know about|tell me about|please know|you (know|should know|need to know)|what.?s your (plan|strategy|approach|idea)|how (are|is) (this|that|it|we)|which.*(should|better|best|prefer)|compare.*(option|approach|model|way)|help me (decide|choose|think|plan|understand)|set which|god prompt)' && ! echo "$task" | grep -qiE '(fix|build|deploy|install|set up|configure|create.*script|write.*code|debug|refactor|migrate|push|commit|merge|checkout|git |npm |pnpm |curl |wget |pip |python3? |bash |node |docker |build|create|generate|scrape|research|analyze|design|launch|execute|produce|make|start|roll out|go ahead|get it done|look into|set up|configure)'; then
    echo "main"
    return 0
  fi
  ...
  # Route to dreami (visual pipeline lead). Excluded: "fix clip factory" (goes to taoz via build rule above)
  ...
  # CODE REVIEW: explicit code review / security audit requests → Taoz code-review mode (Opus)
  # Catches: "review this file", "code review X", "security audit", "check this code for bugs"
  if echo "$task" | grep -qiE '(^code.?review|^review.*(this )?(code|file|script|diff|pr|pull.?request|commit|patch|change)|^security.?audit|^audit.*(this )?(code|file|script)|check.*(this )?(code|file|script).*(bug|security|issue|quality|vuln)|owasp.*(check|audit|review|scan)|^review.*(for )?(bug|security|issue|quality|vuln))' && ! echo "$task" | grep -qiE '(review.*(ad|campaign|brand|creative|content|copy|performance|strategy|market|competitor|revenue)|ad.?review|campaign.?review|brand.?review)'; then
    echo "taoz"
    return 0
  fi

  # TAOZ: thinking, architecture, code review, technical planning → Taoz (needs Opus-level intelligence)
  # Guard: exclude creative brainstorm/campaign planning (dreami), competitor analysis (scout),
  #         visual redesign (dreami)
  ...
  # Note: "bento ugc" is video generation → dreami
  # "bento post/content/caption" → dreami (copywriting)
  if echo "$task" | grep -qiE '(mirra.*(post|content|instagram|recipe|caption|pillar|magic)|create.*mirra.*(post|content)|mirra.*(beyond|rebels?|women|magic))' && ! echo "$task" | grep -qiE 'bento.?ugc'; then
    echo "dreami"
    return 0
  fi

  # COMPARISON ADS: routed via brand-studio (line 122) → dreami → SCRIPT tier
...
agent_to_cost() {
  case "$1" in
    scout)     echo "cheapest (minimax-m2.5)" ;;
    taoz)      echo "cheapest (glm-4.7-flash)" ;;
    # artemis removed - now scout ;;
    dreami)    echo "free (kimi-k2.5)" ;;
    # iris removed - merged into dreami ;;
    # athena removed - merged into main ;;
    # hermes removed - merged into dreami ;;
    # argus removed - spawn subagent ;;
    *)         echo "unknown" ;;
  esac
}
...
# If both override AND router returned catch-all (scout), try Haiku triage
...
if [[ "$SOURCE" = "fallback" || ("$AGENT" = "scout" && "$SOURCE" = "router") || ("$AGENT" = "main" && "$SOURCE" = "router") ]]; then
  # Pre-check: greetings should NEVER hit triage (Haiku inconsistently returns CODE_SIMPLE for greetings)
  if echo "$TASK" | grep -qiE '^(hi|hello|hey|thanks|thank you|ok|yes|no|help|stop|undo|what can you|good (morning|afternoon|evening|night))([,! ]+(zenni|taoz|dreami|scout|there|everyone|team|guys|all|good (morning|afternoon|evening|night)))*[!?., ]*$'; then
    AGENT="main"
    SOURCE="greeting"
...
detect_tier() {
  ...
  # Match greetings with optional agent name, trailing punctuation, or common follow-ups like "good morning"
  if echo "$task" | grep -qiE '^(hi|hello|hey|thanks|thank you|ok|yes|no|help|stop|undo|what can you)([,! ]+(zenni|taoz|dreami|scout|there|everyone|team|guys|all|good (morning|afternoon|evening|night)))*[!?., ]*$'; then
    echo "relay"
    return 0
  fi
  # RELAY: standalone greetings/pleasantries (good morning, good night, etc.)
  if echo "$task" | grep -qiE '^good (morning|afternoon|evening|night)[!?., ]*(zenni|taoz|dreami|scout|there|everyone|team|guys|all)?[!?., ]*$'; then
    echo "relay"
    return 0
  fi

  # RELAY: conversational/discussion messages routed to main by classify_override
  # When Zenni (main) is the target and the message is a discussion/planning/opinion request
  # Zenni should answer herself with GPT-5.4 intelligence, not dispatch to another agent
  # NOTE: action words (look into, roll out, start asap, etc.) removed — those should dispatch, not relay
  if [[ "$agent" = "main" ]] && echo "$task" | grep -qiE '(what do you (think|suggest|recommend)|how should we|what should we|your (opinion|thought|view|take|recommendation)|do you (think|know|remember|understand|agree)|can you (explain|tell me|help me understand|discuss)|let.?s (talk|discuss|plan|think|figure)|please know|know about|tell me about|which.*(should|better|best)|help me (decide|choose|think|plan|understand)|god prompt)'; then
    echo "relay"
    return 0
  fi
  ...
  # LOOKUP: brand info queries — "what is mirra", "tell me about pinxin", "brand info for wholey"
  # Read-only: pull from DNA.json, no agent session needed
  if echo "$task" | grep -qiE '(what (is|are) .*(brand|mirra|pinxin|wholey|serein|rasaya|dr.?stan|gaia.?eats|gaia.?learn|gaia.?print|gaia.?recipes|gaia.?supplements|jade.?oracle|iris brand)|tell me about .*(mirra|pinxin|wholey|serein|rasaya|dr.?stan|gaia)|brand.*(info|detail|dna|about|describe)|about .*(mirra|pinxin|wholey|serein|rasaya|dr.?stan|gaia).*(brand)|(mirra|pinxin|wholey|serein|rasaya|dr.?stan|gaia).*(brand|about|what|who))' && ! echo "$task" | grep -qiE '(create|generate|make|build|fix|change|update|redesign)'; then
    echo "lookup"
    return 0
  fi
  ...
  if [[ "$agent" = "dreami" ]] || [[ "$agent" = "dreami" ]]; then
    ...
    if echo "$task" | grep -qiE '(generate|create|make|compose).*(ad|ads|image|visual|poster|banner|comparison|hero|grid|lifestyle|testimonial|persona|kol|ugc|flash.?sale|boom|promo|deal|discount|urgency|countdown|last.?call|raw|authentic|lo.?fi|meme|price|cod|delivery|product)' && ! echo "$task" | grep -qiE '(batch|bulk|multiple|mass|[0-9][0-9]+ *(ads?|images?|visuals?|posters?|banners?))' && ! echo "$task" | grep -qiE '(video|clip|reel|animation|animate|storyboard|sora|kling|wan|remotion)' && ! echo "$task" | grep -qiE '(ad.?cop(y|ies)|copywriting|marketing.?copy|brand.?copy|sales.?copy)'; then
      echo "script"
      return 0
    fi
    # Bulk generation — ad-bulk-gen.sh handles directly
    if echo "$task" | grep -qiE '(batch|bulk|multiple|mass).*(generat|create|make|image|ad|visual)|([0-9]+)\s*(ads?|images?|visuals?|posters?|banners?)' && ! echo "$task" | grep -qiE '(ad.?cop(y|ies)|copywriting)'; then
      echo "script"
      return 0
    fi
  fi
...
if [[ "$AUTO_DISPATCH" = true ]]; then
  ...
  if [[ "$TIER" = "lookup" ]]; then
    ...
    # Brand info lookup — pull from DNA.json directly
    if echo "$TASK_LOWER" | grep -qiE '(what (is|are)|about|brand.*(info|detail|dna|about)|tell me about)'; then
      if [[ -z "$BRAND" ]]; then
        # Try to detect brand from task even if not caught above
        for b in mirra pinxin wholey serein rasaya dr-stan gaia-eats gaia-learn gaia-print gaia-recipes gaia-supplements jade-oracle iris; do
          if echo "$TASK_LOWER" | grep -qiE "$(echo $b | sed 's/-/.?/g')"; then BRAND="$b"; break; fi
        done
      fi
      if [[ -n "$BRAND" ]]; then
...
print(f\"Brand: {d.get('name',d.get('brand','$BRAND'))}\")
print(f\"Tagline: {d.get('tagline','N/A')}\")
print(f\"Category: {d.get('category','N/A')}\")
print(f\"Voice: {d.get('voice',d.get('tone','N/A'))}\")
print(f\"Audience: {d.get('audience',d.get('target_audience','N/A'))}\")
desc=d.get('description',d.get('about',''))
if desc: print(f\"About: {desc[:300]}\")
" 2>/dev/null || echo "Brand data unavailable for $BRAND")
          echo "LOOKUP:${AGENT}"
          echo "---"
          echo "$RESULT"
          exit 0
  ...
  # COMPARISON ADS: handled by dreami → SCRIPT tier (NanoBanana CLI)
  ...
  # Catches taoz tasks from keyword routing OR classify_override that bypassed triage CODE_TIER detection.
  if [[ "$AGENT" = "taoz" && -z "$CODE_TIER" ]]; then
    # CODE REVIEW detection — explicit code review / security audit → code_review tier (Opus, read-only)
    if echo "$TASK_LOWER" | grep -qiE '(^code.?review|^review.*(this )?(code|file|script|diff|pr|pull.?request|commit|patch)|^security.?audit|^audit.*(this )?(code|file|script)|check.*(this )?(code|file|script).*(bug|security|issue|quality|vuln)|owasp.*(check|audit|review|scan)|^review.*(for )?(bug|security|issue|quality|vuln))' && ! echo "$TASK_LOWER" | grep -qiE '(review.*(ad|campaign|brand|creative|content|copy|performance)|ad.?review)'; then
      CODE_TIER="code_review"
  ...
  if [[ -n "$CODE_TIER" ]]; then
    ...
    case "$CODE_TIER" in
      ...
      code_review) CLI_MODEL="opus" ;;  # Code reviews need Opus-level reasoning
    ...
    echo "AGENT:taoz"
    echo "TASK:${TASK}"
    # code_review uses code-review mode (read-only, structured PASS/FAIL output)
    if [[ "$CODE_TIER" = "code_review" ]]; then
      echo "CMD:bash ${CLAUDE_RUNNER} code-review \"$(echo "$TASK" | sed 's/"/\\"/g')\" . --model ${CLI_MODEL}"
    else
...
# Apply Taoz→CODE upgrade for display (mirrors auto-dispatch logic)
DISPLAY_TIER="$TIER"
if [[ "$AGENT" = "taoz" && "$TIER" = "dispatch" ]]; then
  DISPLAY_TIER="code"
fi
