...
MODE="${1:?Usage: claude-code-runner.sh <review|build|dispatch|inbox> <prompt> [cwd] [budget] [--model sonnet|opus|haiku]}"
...
case "$MODE" in
  ...
  dispatch)
    ...
    KSYNC_EXCERPT=""
    if [ -f "$KSYNC" ]; then
      KSYNC_EXCERPT=$(head -c 10000 "$KSYNC" 2>/dev/null || echo "")
    fi
...
- Read KNOWLEDGE-SYNC.md at ~/.openclaw/workspace-taoz/KNOWLEDGE-SYNC.md for accumulated learnings.
