...
case "$MODE" in
  ...
  build)
    ...
    echo "---"
    env -i PATH="/Users/jennwoeiloh/.local/bin:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin" HOME="$HOME" ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}" claude -p --model opus --system-prompt "You are a Skill Builder for GAIA CORP-OS. Write clean, tested code. Return EXACTLY these sections: (1) Result — what was built/changed (1-3 lines), (2) Proof — build logs or test output, (3) What changed — list of files, (4) Learning — 1 line on what to improve next time." --allowedTools "Bash,Edit,Read,Write,Glob,Grep" --max-budget-usd "$BUDGET" --add-dir "$CWD" "$PROMPT"
    echo ""
...
" 2>/dev/null
    ;;
  *)
    echo "ERROR: Unknown mode '$MODE'. Use 'review', 'build', 'dispatch', or 'inbox'."
    echo "Usage: claude-code-runner.sh <review|build|dispatch|inbox> <prompt> [cwd/from] [budget/room]"
    exit 1
    ;;
esac
