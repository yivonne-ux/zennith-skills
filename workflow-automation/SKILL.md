...
Enable GAIA agents to autonomously register new output types, production workflows, and skills into the production system. When Artemis discovers a new ad format from competitor analysis, or Dreami invents a new creative format, they dispatch to this skill to register it — no human CLI input required.
...
- **Artemis** finds new format via competitor scraping -> pipes JSON to register-output-type.sh
- **Dreami** invents creative format -> pipes JSON to register-output-type.sh
- **Iris** discovers new visual style -> pipes JSON to register-output-type.sh
- **Taoz** runs update-skill-registry.sh after any skill change
- **Myrmidons** runs update-skill-registry.sh nightly via cron

## Agent Dispatch Examples

```
# Artemis discovers a competitor's "reaction video" format
{
  "id": "reaction-video",
  "name": "Reaction Video",
